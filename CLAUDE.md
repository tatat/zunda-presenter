...
- `scripts/ctl.mjs` — server lifecycle + playback control wrapper (run from the project dir; `PRESENTER_DECKS_DIR` to override the decks root): discovers the server port from `server.json`, verifies project identity via `/api/info`, then `start` (idempotent, spawns detached, logs to `runtime/`) / `stop` / `info` / `state` / `open` / `play` / `pause` / `goto` / `chars`. Exists to give the whole server interaction one stable, allowlistable command surface instead of port-embedding curls and port-scanning loops.
