...
- `scripts/ctl.mjs` — playback control wrapper (run from the project dir; `PRESENTER_DECKS_DIR` to override the decks root): discovers the server port from `server.json`, verifies project identity via `/api/info`, then `info`/`state`/`open`/`play`/`pause`/`goto`/`chars`. Exists to give control a stable, allowlistable command surface instead of port-embedding curls.
