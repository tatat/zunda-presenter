...
1. Check the size (`ls -lh`). GitHub's attachment limits for video are 10MB on free plans and 100MB on paid plans (docs.github.com, "Attaching files"); if the file exceeds the repo's limit, re-export smaller: `PRESENTER_VIDEO_HEIGHT=720 node ${CLAUDE_PLUGIN_ROOT}/scripts/export-video.mjs`.
