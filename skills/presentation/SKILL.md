...
5. Audit the readings — step 2's `npm run preflight` already produced the full readings output (standalone: `npm run readings`, works before anything is synthesized) — and fix what is actually misread (see Readings).
...
Also run the full readings audit (`npm run preflight` — see Readings): the synth log alone misses every cached line.
