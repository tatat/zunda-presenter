...
A slot with no claim to argue is dropped, not filled. What sets the count is the question chain (structure.md): a beat exists because the one before it planted a question this Subject must answer for this Reader, and the deck ends where nothing further is planted. In a chaptered deck the chapters come from the Subject's parallel items and the chain runs inside each; a chapter's first beat is planted by its chapter question. Length is derived from that, never targeted, and a long deck owes no justification for being long (structure.md, Detail Discipline).
