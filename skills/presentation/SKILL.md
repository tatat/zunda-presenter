...
Do this before creating the deck dir: state both back to the user in one message and wait for their answer. Unconditional.

- **Subject** — what the deck is about, quoted in the requester's own words. The spine may sharpen it into a claim; it may never replace it with a different topic. A request for what something *is* does not license a deck about how it came to be: the work — how you found the material, what you verified, what a review changed — goes in `context.md`, never the spine.
- **Reader** — who watches, stated as what they do and do not know. Default: fluent in the domain, absent from the work. Name the domain explicitly; that is the part that varies per deck.

Record both verbatim at the top of `outline.md` (`Subject:` / `Reader:` — template in structure.md) before writing the spine.

A calling skill may fix them for its format instead (`pr-video` does). Those are then the agreement: copy them and go on, returning to the user only if the request contradicts them. Never supply them yourself when they are neither agreed nor fixed.

## Workflow

1. Create a new deck dir (or reuse an existing one for updates) and write `<deck-name>/outline.md` **before any dialogue** — spine (one-sentence takeaway), entry question, and beats (`holds/shows/claim/plants`; roughly one per content slide, but beats need not map 1:1 to slides). Read `references/structure.md` first: it defines the format, the question-chain arc, and the failure modes the outline exists to catch (source-order transcription, detail drowning the spine, topics instead of beats). **Read references with the Read tool, in full** — excerpting with sed/head/grep on a first read silently loses the tail, and the tail is where the load-bearing parts sit (the role docs end in the pre-emit Silent Checks); each reference ends with an end-marker line naming itself, so a read that never reached it was an excerpt (grep to re-consult a specific section later is fine). Structural problems cost one line to fix here and twenty after the dialogue exists. Copy the agreed `Subject` and `Reader` in first, then write the spine as a claim about that subject — if the spine is pulling toward a different topic, that is not a writing decision to make alone: go back and re-agree with the user. Decide chapters here too, and which kind: a subject that is several independent things takes one **parallel** chapter per item, where the chain stops at the boundaries and the weighting rules are read per chapter; a long single argument may take **sequential** chapters, which are grouping only (structure.md, Detail Discipline and Weighting the Beats).
   ...
   **Re-gate whenever the outline's structure changes again.** The version that applied the findings is a new outline no gate has read. The trigger is any edit to what this gate reads: `Subject`, `Reader`, the spine, the entry question, the chapter set and its questions, and any beat's `holds` / `shows` / `claim` / `plants` / `depth` / `dig`. Re-spawn after applying this gate's own findings, and again when a later gate forces a structural rewrite (structure.md, Lifecycle: the outline is updated first in that case). Edits outside those fields — prose in the evidence block, the cut list, notes to yourself — need no second pass.
...
3. **Naive-reader review** — you just absorbed the source material, so you cannot reliably simulate not knowing it; only a reader without the knowledge can (curse of knowledge). After the dialogue draft, extract the dialogue alone with `node ${CLAUDE_PLUGIN_ROOT}/scripts/view-deck.mjs --dialogue` and spawn the plugin's **`naive-reader` subagent** with the agreed `Reader` line followed by that text — and nothing else. No title, no `context.md`, no source docs, no hint of expected findings (the review questions live in the agent definition), and **never the `Subject`**, which is what the review measures. The `Reader` line leaks nothing and is what makes the triage below decidable: it separates the field's words from yours. **Run it in the background and continue with steps 4–6 while it works** — changed lines re-synthesize cheaply, so synthesizing before the review lands wastes nothing.
   ...
   **Compare the reviewer's recovered main claim against the recorded `Subject`.** Matching is the pass, not recovery. A recoverable claim about something else means the deck drifted: fix the spine, not the lines. Of two competing claims, keep the one matching the `Subject`. In a deck of parallel chapters the report gives the deck-level claim and one per chapter: compare the first against the `Subject` and the rest against the chapters; competition inside one chapter is the finding. For the flow-check part of the report, the interpretation guide is `references/structure.md` (Blind Flow Check): an unrecoverable main claim or repeatedly violated boundary expectations mean the outline needs fixing, not individual lines; a template-rhythm flag means re-varying how beats surface, not patching lines. A boundary-expectation violation is not dismissible on being a one-off — reread the seam's two sides before judging even a single one. Check the reviewer's section force-ranking against the outline's `depth` markings and its time-to-purpose answer against the Stakes slot (both interpreted in the same guide). Repeat after any major rewrite — rewrites reintroduce the same blind spot.
...
- **No `context.md` / Web Q&A prep** (step 4). The Q&A agent only answers questions typed into the player, so nothing would read it. "Cut = relocate to `context.md`" therefore does not hold here: cut on the same criterion (does any beat's question need it) and accept the loss. If the deck is later opened in the player, write `context.md` then.

Every gate runs as written, naive-reader included (the readings audit's delegation rule already scales with deck length — see Readings).
...
A slot with no claim to argue is dropped, not filled. What sets the count is the question chain (structure.md): a beat exists because the one before it planted a question this Subject must answer for this Reader, and the deck ends where nothing further is planted. Parallel chapters come from the Subject's independent items, and the chain runs inside each: such a chapter's first beat is planted by its chapter question, not by the chapter before. Length is derived from that, never targeted, and a long deck owes no justification for being long (structure.md, Detail Discipline).

Sizing is a prediction to check the derivation against, not a budget: ~1 slide per idea, 3–6 lines per slide, so a 5-slide deck lands near 20–25 lines ≈ 2–3 minutes. A deck far off its own beat count in either direction means beats were invented or real claims compressed out; fix the outline, not the line count.
...
- **Write speech, not copy.** Every line is the plainest way a person would actually say the thing. 倒置 (clefting into 「〜のは〜」), 名詞化 (a noun phrase for a clause, a 〜止まり-style compound) and 省略 compress Japanese cheaply and move the line into written register. One lands; a run of them does not. Test by saying the line aloud.
- Each line ≤ 60 characters. Long lines sound monotonous and make seeking coarse. **Split rather than cram — nominalizing or eliding to fit is cramming.** A thought that only fits as a noun phrase is two lines. Short is not the goal, plain is.
