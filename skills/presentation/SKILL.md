...
**Paths**: each presentation is its own deck directory `<project>/.zunda-presenter/<deck-name>/` (kebab-case slug for the topic, e.g. `auth-refactor-plan`) containing `script.json`, `audio/`, and its own `dictionary.json`. (Legacy, deprecated: with no deck-local dictionary, a shared `.zunda-presenter/dictionary.json` applies instead — selection, not merge; when adding a local one, copy over any shared entries the deck still needs.) The player URL is `http://localhost:3939/d/<deck-name>`. Plugin code is at `${CLAUDE_PLUGIN_ROOT}`. Synthesis command (used throughout):

```
PRESENTER_DECK_DIR="<abs project path>/.zunda-presenter/<deck-name>" node ${CLAUDE_PLUGIN_ROOT}/scripts/synthesize.mjs
```

**Deck file access**: inspect and edit deck files with the tools this workflow already names — `view-deck` for playback order and ids, Read/Grep for content, Edit/Write for changes (including `dictionary.json`). No ad-hoc shell: `node -e` and heredoc redirects can't be allowlisted (every one prompts the user), and redirects clobber files unread.

## Before the workflow: agree on the subject and the reader

Do this before creating the deck dir: state both back to the user in one message and wait for their answer. It is not conditional on feeling uncertain. A substituted subject is experienced as insight, not as doubt, so your own confidence is not evidence that you kept the one you were given.

- **Subject** — what the deck is about, quoted in the requester's own words. Their words bound it, and the boundary that gets crossed most easily is artifact vs. work: a request for what something *is* is not a licence for a deck about how it came to be. The spine may sharpen the subject into a claim; it may never replace it with a different topic. The work itself — how you found the material, what you verified, what a review changed — has a home already (`context.md`, served on demand by Q&A) and never becomes the spine.
- **Reader** — who watches, stated as what they do and do not know. Default: fluent in the domain, absent from the work. They know the field's tools and vocabulary; they do not know what you did, decided, or found. Name the domain explicitly, since that is the part that varies per deck.
...
1. Create a new deck dir (or reuse an existing one for updates) and write `<deck-name>/outline.md` **before any dialogue** — spine (one-sentence takeaway), entry question, and beats (`holds/shows/claim/plants`; roughly one per content slide, but beats need not map 1:1 to slides). Read `references/structure.md` first: it defines the format, the question-chain arc, and the failure modes the outline exists to catch (source-order transcription, detail drowning the spine, topics instead of beats). **Read references with the Read tool, in full** — excerpting with sed/head/grep on a first read silently loses the tail, and the tail is where the load-bearing parts sit (the role docs end in the pre-emit Silent Checks); each reference ends with an end-marker line naming itself, so a read that never reached it was an excerpt (grep to re-consult a specific section later is fine). Structural problems cost one line to fix here and twenty after the dialogue exists. Copy the agreed `Subject` and `Reader` in first, then write the spine as a claim about that subject — if the spine is pulling toward a different topic, that is not a writing decision to make alone: go back and re-agree with the user.

   **Motive claims need evidence.** When the spine or a beat's `claim` says *why* something happened, the outline must name the evidence: a doc, a commit, the user's words. No later stage checks facts against the source, so an invented-but-plausible why survives every review (measured: one collapsed on the deck owner's first question, invalidating six lines, the outline, and context.md). If the source shows no why, ask the user instead of manufacturing one — a deck without a causal claim is legitimate.

   Then gate the outline through the plugin's **`outline-checker` subagent** — inputs per its definition: outline path, structure.md path, a one-paragraph framing (audience + goal), never the source. Spawn it in the background like the others — the gate is on dialogue, not on your turn: read step 2's role references while it runs (they don't depend on the outline), and **wait for its result before writing any dialogue**. When overlap work runs out, just end the turn — completion resumes you; don't poll or idle. Apply or dismiss each finding with a reason; its coverage questions are yours to check against the source — the one thing it can't see.

   **Re-gate whenever the outline's structure changes again** — the same rule the naive-reader review carries, and it is easy to skip here because the outline looks finished once the findings are applied. It isn't: the version that fixed them is a new outline that nobody has read, and a fix to one seam routinely breaks the next. The trigger is any edit to what this gate reads: `Subject`, `Reader`, the spine, the entry question, and any beat's `holds` / `shows` / `claim` / `plants` / `depth` / `dig` — moving a `peak` marker counts, because the weighting profile is one of the things it checks. Re-spawn after applying this gate's own findings, and again when a later gate (naive-reader especially) forces a structural rewrite, since the outline is updated first in that case anyway (structure.md, Lifecycle). Edits outside those fields — prose in the evidence block, the cut list, notes to yourself — need no second pass.
2. Before writing any dialogue, read the role references in full — `references/roles/metan.md`, `references/roles/zundamon.md`, `references/roles/interaction.md` (see Dialogue rules below; already done during step 1's gate wait). Then write `.zunda-presenter/<deck-name>/script.json` against the outline (reference below) — as a skeleton, not a script: never transcribe `holds`/`plants` into dialogue verbatim (structure.md, From Outline to Dialogue). The browser hot-reloads the open deck on every save. Validate with `node ${CLAUDE_PLUGIN_ROOT}/scripts/check-deck.mjs` (same env var as the synth command; also covers `qa.json` — see Web Q&A): JSON syntax, required fields, id uniqueness, slide references, enum values — errors mean invalid deck structure or values, fix them all (some break playback/synthesis, others the runtime papers over with fallbacks); warnings flag guideline violations and likely typos. After any structural edit (inserting/reordering lines, splitting slides), re-check the deck's shape with `node ${CLAUDE_PLUGIN_ROOT}/scripts/view-deck.mjs`: it prints the lines in **actual playback order** (array position — the only thing that determines order; ids drift from story order after edits and the slides array orders independently), grouped by slide, flagging same-speaker runs mid-slide (often an insert that landed out of order). Pass slide ids to restrict output while keeping indices absolute: `node ${CLAUDE_PLUGIN_ROOT}/scripts/view-deck.mjs s3 s4`. Once the draft is complete, run `node ${CLAUDE_PLUGIN_ROOT}/scripts/preflight.mjs` (same env var): check-deck, view-deck, and the readings audit in one command — its readings section is exactly the input step 5's auditors need.
3. **Naive-reader review** — you just absorbed the source material, so you cannot reliably simulate not knowing it; only a reader without the knowledge can (curse of knowledge). After the dialogue draft, extract the dialogue alone with `node ${CLAUDE_PLUGIN_ROOT}/scripts/view-deck.mjs --dialogue` and spawn the plugin's **`naive-reader` subagent** with the agreed `Reader` line followed by that text — and nothing else. No title, no `context.md`, no source docs, no hint of expected findings (the review questions live in the agent definition), and above all **never the `Subject`**: whether a claim reaches the viewer is the thing being measured, and a reviewer told the answer recites instead of recovering. The `Reader` line leaks nothing and buys the triage below: it tells the reviewer which words are the field's (not findings) and which are yours (findings). **Run it in the background and continue with steps 4–6 while it works** — changed lines re-synthesize cheaply, so synthesizing before the review lands wastes nothing.
   ...
   Apply or dismiss-with-reason every finding, including the flow-check / force-ranking / time-to-purpose sections — a finding handled neither way is the one the viewer raises later (measured: a silently dropped force-ranking finding became the viewer's first complaint). Triage the flags with your full context: the reviewer can't see the slides, so a flag that the visible slide resolves (deixis like これが全体図よ pointing at a diagram) is dismissible — but an unintroduced term is not resolved by a slide, because slides carry keywords, they don't introduce anything. Judge unintroduced terms against the agreed `Reader`: a word belonging to the field they are fluent in needs no introduction (glossing it costs lines and crowds out the subject), while anything from the work itself does. Patch a short introduction line before each surviving first use (or cut the term).

   **Compare the reviewer's recovered main claim against the recorded `Subject`.** Recovery is not the pass — matching is. A claim that is recoverable but about something else means the deck drifted, and the fix is the spine, not the lines that expose it. When the report names two competing claims, the one that matches the `Subject` is the one to keep; deleting it to make the other consistent is the failure this comparison exists to stop. For the flow-check part of the report, the interpretation guide is `references/structure.md` (Blind Flow Check): an unrecoverable main claim or repeatedly violated boundary expectations mean the outline needs fixing, not individual lines; a template-rhythm flag means re-varying how beats surface, not patching lines. A boundary-expectation violation is not dismissible on being a one-off — reread the seam's two sides before judging even a single one. Check the reviewer's section force-ranking against the outline's `depth` markings and its time-to-purpose answer against the Stakes slot (both interpreted in the same guide). Repeat after any major rewrite — rewrites reintroduce the same blind spot.
...
5. Audit the readings — step 2's `node ${CLAUDE_PLUGIN_ROOT}/scripts/preflight.mjs` already produced the full readings output (standalone: `node ${CLAUDE_PLUGIN_ROOT}/scripts/synthesize.mjs --readings`, works before anything is synthesized) — and fix what is actually misread (see Readings).
6. Run the synth command above — synthesizes all lines and fills in `audio` fields. Cached by content hash, so only changed lines re-synthesize; iterate freely. Re-audit after any edit (see Readings). Synthesis rewrites `script.json` on disk (fills `audio`, reformats), so re-read it before any later edit.
...
- **No `context.md` / Web Q&A prep** (step 4). Nothing would read it: the Q&A agent only answers questions typed into the player. This also changes what cutting a detail means everywhere else in this file and in structure.md — "cut = relocate to `context.md`" assumes a Q&A agent that will serve it, and a light deck has none, so a cut detail is simply gone. Cut on the same criterion regardless (does any beat's question need it), and accept the loss; if the deck is later opened in the player, write `context.md` then.
...
If a draft deck is later kept, exported, or shown wider, run it through light or full first.
...
**Standard structure** — a menu of slots that commonly earn a place, not a checklist to fill:
...
A slot with no claim to argue is dropped, not filled — silently, with nothing recorded, because it was never owed a slide. What sets the count is the question chain (structure.md): a beat exists because the one before it planted a question this Subject must answer for this Reader, and the deck ends where nothing further is planted that the Subject owes. Run that and a narrow subject yields a few beats although six slots are listed above, while a broad one yields many and owes no justification for the length (structure.md, Detail Discipline: length is an order, shape is unconditional). Length is derived, never targeted.

Sizing, therefore, is a prediction to sanity-check the derivation against — not a budget to spend: ~1 slide per idea, 3–6 lines per slide, so a 5-slide deck lands near 20–25 lines ≈ 2–3 minutes. A deck far off its own beat count in either direction means beats were invented to fill slots, or real claims were compressed out; fix the outline, not the line count.
...
- **Write speech, not copy.** Unless the deck asks for something else, every line is the plainest way a person would actually say the thing. The pull runs the other way: 倒置 (clefting a clause into 「〜のは〜」), 名詞化 (a noun phrase standing in for a clause, or a 〜止まり-style compound), and 省略 (a sentence held together by what it leaves out) are the cheapest ways to compress Japanese — and each one moves the line toward written or advertising register, a sentence nobody utters. One such construction lands; a run of them is exhausting, and viewers report the unnaturalness before they report anything else. Test by saying the line aloud.
- Each line ≤ 60 characters. Long lines sound monotonous and make seeking coarse. **Split rather than cram — and compressing by nominalizing or eliding is cramming**, even when the result fits. A thought that only fits as a noun phrase is two lines. Short is not the goal, plain is, and the two are easily confused: an edit made to lighten a line is where the register slips back.
...
- `lines[].id` must be **stable and unique** — playback position survives edits by id. Never renumber existing ids; inserted lines get fresh ids (`q1a`, `fix3`). Ids therefore drift from story order over time — that's expected; `node ${CLAUDE_PLUGIN_ROOT}/scripts/view-deck.mjs` shows the real order.
...
**Audit:** the synth log prints the engine's actual reading (`reading: …`) for each **newly synthesized** line — scan it on every run. Cached lines pass silently, so audit every line with `node ${CLAUDE_PLUGIN_ROOT}/scripts/synthesize.mjs --readings` (below): it prints each line's reading with dictionary and `spoken` applied, synthesizing nothing. Judge by deriving each line's reading from its grammar and meaning and diffing against the kana — your derivation is the reference, because a misreading is still a valid reading of some segmentation of the characters (the engine never outputs gibberish), so scanning its kana for plausibility catches only garbled English and dropped symbols. Run it after writing the script, before declaring the deck ready, and again at the end of any editing session — at these checkpoints `node ${CLAUDE_PLUGIN_ROOT}/scripts/preflight.mjs` bundles it with check-deck and view-deck:

```
PRESENTER_DECK_DIR="<abs project path>/.zunda-presenter/<deck-name>" node ${CLAUDE_PLUGIN_ROOT}/scripts/synthesize.mjs --readings
```

**Delegate the judging** — reading judgment is line-local, so split it across the plugin's **`reading-auditor` subagents**: one per ~10–15 lines, spawned in parallel, each given its inputs per the definition (the FULL readings output, the deck's `dictionary.json` content, the plugin root and deck dir paths, and its assigned range). They return try-reading-verified findings — including ones marked `possibly intentional — confirm`, which only you may dismiss (you know the deck's intent; the auditor doesn't). You apply the fixes (dictionary / `spoken`), re-synth, and close the editing session with one final `node ${CLAUDE_PLUGIN_ROOT}/scripts/preflight.mjs`. Kick the auditors off in the background together with the naive-reader review (workflow step 3) and write `context.md` while both run. For a short deck (≲15 lines), just judge the output yourself.

**Probe a string** — to check a term's reading (before registering a dictionary entry, or when a reading feels off) without editing anything, `node ${CLAUDE_PLUGIN_ROOT}/scripts/reading-tools.mjs try "<text>"` (same env vars as above) prints the raw and dictionary-applied readings side by side, in the same kana display as `node ${CLAUDE_PLUGIN_ROOT}/scripts/synthesize.mjs --readings`. Several probes batch into one invocation with `--` separators — `node ${CLAUDE_PLUGIN_ROOT}/scripts/reading-tools.mjs try 偽陽性 -- "設計の要"` — so collect the terms first and probe once, not one process per term. **Evaluate a candidate entry by probing, never by registering**: probe the key and the intended value as two plain strings (`-- "設計の要" -- "設計のカナメ"`) and compare the readings — the same method `check-dictionary` applies to registered entries. Writing candidates into `dictionary.json` as a scratchpad and resetting it later is how a needed entry gets lost (measured: a deck's own showcase entry shipped broken exactly this way).

**Audit the dictionary itself** — `node ${CLAUDE_PLUGIN_ROOT}/scripts/reading-tools.mjs check-dict` reads every entry's key both raw and with the replacement value, and prints the two kana outputs per entry. A katakana value is not automatically safer than the original: the engine may read the raw term correctly and the "fix" worse (measured: raw `VPC` → ブイピーシー correct, but the replacement string ブイピーシー renders シー as スィー — the entry made things worse). Drop entries flagged `[no effect]` (identical reading *and* accent structure — the comparison keeps VOICEVOX's accent markers, so an accent-only difference still counts as a change) and entries whose replaced reading is worse than raw. Run it when inheriting an existing dictionary (a deck's or a legacy shared one) or after a batch of additions.
...
- A term that should read the same throughout the deck → **`<deck-name>/dictionary.json`**: `"term": "読み"` per term. Subtitles keep the original spelling; only the audio uses the reading (longest match first, case-insensitive). The value need not be all-katakana — keep the parts the engine already reads correctly in their original script and respell only the broken part: `"設計の要": "設計のカナメ"`, not `"セッケイノカナメ"` (hand-written katakana can miss the engine's own normalization — measured: a written セッケイ keeps a short イ where the engine reads 設計 as セッケエ; same family as the VPC regression in check-dictionary). English/technical terms and specialist kanji vocabulary (e.g. 偽陽性 → ニセヨウセイ instead of ギヨウセイ) are the usual offenders — expect most entries to come from them, but register what the audit showed broken, not what looks risky. Replacement is a plain substring match, which cuts both ways: keys may be whole phrases, and **context-dependent readings need the context in the key** — for 要→カナメ register `"設計の要": "セッケイノカナメ"`, never bare `"要": "カナメ"`, which would corrupt 必要/要素 into 必カナメ/カナメ素. Never register a single ambiguous kanji without surrounding context. Register multi-word English names whole (`"Firebase Hosting": "ファイアベースホスティング"`), never the lead word alone — partial replacement leaves Latin words bordering katakana, which misreads unpredictably (measured: `Hostingに` → エイチ・オスティングニ regardless of spacing, while `Planに` read fine; the whole-term entry reads correctly).
...
- **Inserted and corrected lines follow the outline's evidence rule** (workflow step 1) — splices skip every review gate. Assert only what the source or the user has shown; mark conjecture as conjecture (「〜かもしれないわ」).
...
Verify layout without touching the user's tab: spawn the plugin's **`slide-checker` subagent** with the plugin root and deck dir paths, in the background once the slides are content-stable, and do the wrap-up work meanwhile. It runs `node ${CLAUDE_PLUGIN_ROOT}/scripts/snap-deck.mjs` itself (needs playwright — one-time `npx playwright install chromium` in the plugin root — but not the server). Apply its findings, then re-spawn (slide-id restriction via `node ${CLAUDE_PLUGIN_ROOT}/scripts/snap-deck.mjs s3 s5` keeps re-checks cheap). To eyeball one slide in a real tab instead, `http://localhost:3939/d/<deck-name>#preview:N` jumps straight to line index N with no click-overlay.

Also run the full readings audit (`node ${CLAUDE_PLUGIN_ROOT}/scripts/preflight.mjs` — see Readings): the synth log alone misses every cached line.
