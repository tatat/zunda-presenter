/* Reading audit helpers — query VOICEVOX readings without synthesizing or
   ...
   try:        node scripts/reading-tools.mjs try "<text>"   (or PRESENTER_TEXT="<text>")
     ...
     engine session instead of a loop of invocations:
       node scripts/reading-tools.mjs try 偽陽性 -- "設計の要" -- ctl.mjs

   check-dict: node scripts/reading-tools.mjs check-dict
...
// probe they are joined, so `node scripts/reading-tools.mjs try null を返すのだ` audits
...
if (mode === "try" && tryTexts.length === 0) {
  console.error(`usage: node ${process.argv[1]} try "<text>" [-- "<text>" ...]  (or PRESENTER_TEXT="<text>")`);
  process.exit(1);
}
...
// Same display form as the readings audit: markers stripped, phrase spacing kept
