...
- **Derive, then diff — plausibility is not verification.** For each line, derive the reading its grammar and meaning demand, then compare against the engine's kana; a divergence is a finding even when the engine's version is a valid reading of the characters involved. Both shipped misses came from skipping the derivation and accepting character-level plausibility: a function word absorbed into a neighboring token (topic は must read ワ; measured: 理由はね → リユウ ハネ), and a multi-reading word given the character's commoner reading instead of the word's (measured: 対になって → タイニ, but the word 対になる reads ツイ). Fixes follow scope as usual: a misread function word needs `spoken` (a dictionary key can't safely hold one bare), a misread word a contextual dictionary key.
